;jQuery(document).ready(function($){
	...
	var parseJson = function(results, $widget) {
		...
		// $widget.find('.wordpress-rating').html( "Rating: " + rating + " out of 5 stars <!-- " + results.num_ratings + " -->" );
	...
	$('.wordpress-widget').each(function(){
		...
		if (i == 0) $('head').append(
			...
			+'.wordpress-box .wordpress-box-title .wordpress-stats{position:absolute;top:8px;right:10px;background:white;border:1px solid #ddd;border-radius:3px;font-size:11px;font-weight:bold;line-height:21px;height:21px;padding:0 .5em}'
			...
			+'.wordpress-box .wordpress-box-title .wordpress-stats .watchers{border-right:1px solid #ddd}'
			+'.wordpress-box .wordpress-box-title .wordpress-stats .forks{background-position:-4px -21px;padding-left:15px}'
			...
			+'.wordpress-stars-wrap, .wordpress-stars {background:url(http://wordpress.org/extend/plugins-plugins/bb-ratings/stars.png) bottom left}'
