;jQuery(document).ready(function($){
	...
	var parseJson = function(results, $widget) {
		...
		// instead of watchers and forks we use downloads and num of reviews
		$widget.find('.watchers').text( ( (results.rating / 100) * 5).toFixed(1) );
		$widget.find('.forks').text( results.num_ratings );

		$widget.find('.description span').text(results.short_description);
	...
	$('.wordpress-widget').each(function(){
		...
		$widget = $(
			...
			+'<a class="watchers" href="' + pluginUrl + '/watchers">?</a>'
			+'<a class="forks" href="' + pluginUrl + '/network/members">?</a>'
			+'</div>'
			+'</div>'
			+'<div class="github-box-content">'
			+'<p class="description"><span></span> &mdash; <a href="' + pluginUrl + '#readme">Read More</a></p>'
			+'<p class="link"></p>'
			+'</div>'
